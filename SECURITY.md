...
9. **Schema `pattern` is trusted author input**: JSON Schema `pattern` / `patternProperties` regexes come from the application developer's schema, not from the request. The zero-DOM fast path compiles them with `fancy-regex`, which delegates regular (non-backtracking) patterns to the linear `regex` engine and runs patterns that require the backtracking VM (lookaround / backreferences) under a **1,000,000-step cap** (its default). A cap miss is a non-match on the fast path and a `backtrack_limit` validation error on the authoritative `jsonschema` fallback — the same accept/reject outcome; the fallback uses the **same** engine and cap, so routing a pattern to it is **not** a mitigation. Classic nested-quantifier patterns such as `^(a+)+$` are regular and therefore evaluated in linear time even against large inputs. The cap is load-bearing where the VM is actually entered: with a backreference and nested quantifiers (e.g. `^((a+)+)+\1$`) the pattern is exponential without the cap (over 10 s already at 30 characters) and a constant ~25 ms at 50 KB with it. However, a hard construct over a regular inner (e.g. `(?=(a+)+$)a`) does **not** enter the VM for the inner quantifier and is **not** bounded by the cap — it costs **quadratic** time in the validated string length (measured ~4× per doubling: 5 KB ≈ 33 ms, 20 KB ≈ 0.5 s, 50 KB ≈ 3.3 s, extrapolating to tens of minutes for a 1 MiB body). Treat schema regexes as trusted code: avoid lookaround / backreference-heavy patterns, and keep `maxBodyBytes` well below the 1 MiB default on schema-validated routes whose patterns are not simple/linear. Regression-pinned by `test/unit/ingress/regex-dos.test.ts`.
