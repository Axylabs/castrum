// rust/core/headers.rs — HeaderRefs: zero-alloc packed header parser
// Pure Rust, no napi dependencies. Uses CoreResult instead of napi::Result.
...
impl<'a> HeaderRefs<'a> {
    /// Create an empty `HeaderRefs` with no headers present.
    #[inline(always)]
    pub fn empty() -> Self {
        Self {
            origin: None,
    ...
    #[inline]
    pub fn parse(packed: &'a [u8], is_options: bool, max_headers: usize) -> CoreResult<Self> {
        ...
        if count > max_headers {
            return Err(too_many_items(max_headers, count));
        }
        ...
        for _ in 0..count {
            if pos + 2 > packed.len() {
                return Err(malformed_data("packed header name length", pos));
            }
            ...
            if pos + name_len > packed.len() {
                return Err(malformed_data("packed header name", pos));
            }
            ...
            if pos + 4 > packed.len() {
                return Err(malformed_data("packed header value length", pos));
            }
            ...
            if pos + value_len > packed.len() {
                return Err(malformed_data("packed header value", pos));
            }
    ...
    /// Get the Cookie header value, if present.
    #[inline(always)]
    pub fn cookie(&self) -> Option<&[u8]> {
        self.cookie
    }

    /// Get the X-Forwarded-For header value, if present.
    #[inline(always)]
    pub fn xff(&self) -> Option<&[u8]> {
        self.xff
    }

    /// Get the X-Real-IP header value, if present.
    #[inline(always)]
    pub fn x_real_ip(&self) -> Option<&[u8]> {
        self.x_real_ip
    }

    /// Get the X-Forwarded-Proto header value, if present.
    #[inline(always)]
    pub fn x_forwarded_proto(&self) -> Option<&[u8]> {
        self.x_forwarded_proto
    }

    /// Get the Access-Control-Request-Method header value, if present.
    #[inline(always)]
    pub fn acrm(&self) -> Option<&[u8]> {
        self.acrm
    }

    /// Get the Access-Control-Request-Headers header value, if present.
    #[inline(always)]
    ...
    /// Returns `true` if the Cookie header is present.
    #[inline(always)]
    pub fn has_cookie(&self) -> bool {
        (self.flags & HAS_COOKIE) != 0
    }

    /// Returns `true` if the X-Forwarded-Proto header is present.
    #[inline(always)]
    pub fn has_xfp(&self) -> bool {
        (self.flags & HAS_XFP) != 0
    }

    /// Returns `true` if the ACRM header is present.
    #[inline(always)]
    pub fn has_acrm(&self) -> bool {
        (self.flags & HAS_ACRM) != 0
    }

    /// Returns `true` if the ACRH header is present.
    #[inline(always)]
...
mod tests {
    ...
    fn make_packed(name: &str, value: &str) -> Vec<u8> {
        let mut buf = Vec::with_capacity(2 + 2 + name.len() + 4 + value.len());
    ...
    fn test_parse_origin() {
        let packed = make_packed("origin", "https://example.com");
        let h = HeaderRefs::parse(&packed, false, 100).unwrap();
        assert!(h.has_origin());
        assert_eq!(h.origin(), Some(b"https://example.com" as &[u8]));
    }

    #[test]
    fn test_parse_cookie() {
        let packed = make_packed("cookie", "session=abc123");
        let h = HeaderRefs::parse(&packed, false, 100).unwrap();
        assert!(h.has_cookie());
        assert_eq!(h.cookie(), Some(b"session=abc123" as &[u8]));
    }
