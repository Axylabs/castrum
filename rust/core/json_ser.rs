// rust/core/json_ser.rs — JSON serialization helpers
...
/// Determine whether bytes are valid UTF-8.
#[inline(always)]
        ...
        while i < remaining.len() {
            ...
            match b {
                ...
                _ => {
                    ...
                    let next_special = remaining_slice.iter().position(|&c| c < 0x20);
                    match next_special {
                        Some(n) => {
...
#[inline]
pub fn cookie_json_into_slice(input: &[u8], out: &mut [u8], max_pairs: usize) -> CoreResult<usize> {
    if out.len() < 2 {
        return Err(buffer_too_small(2, out.len()));
    }
    ...
    for pair in input.split(|&b| b == b';') {
        ...
        let end = pos.checked_add(needed).ok_or_else(|| overflow("cookie JSON"))?;
        if end > out.len() {
    ...
    let end = pos.checked_add(1).ok_or_else(|| overflow("cookie JSON closing brace"))?;
    if end > out.len() {
        return Err(buffer_too_small(end, out.len()));
    }
...
pub fn packed_pairs_to_json_into_slice(
    ...
    let count = crate::core::util::read_u32_le(packed, 0)? as usize;
    ...
    for _ in 0..count {
        ...
        let key_len = crate::core::util::read_u32_le(packed, src)? as usize;
        src += 4;

        if src + key_len > packed.len() {
            return Err(malformed_data("packed pairs: truncated key", src));
        }

        let key = &packed[src..src + key_len];
        src += key_len;

        let val_len = crate::core::util::read_u32_le(packed, src)? as usize;
        src += 4;

        if src + val_len > packed.len() {
            return Err(malformed_data("packed pairs: truncated value", src));
        }
        ...
        let end = pos.checked_add(needed).ok_or_else(|| overflow("pairs JSON"))?;
        if end > out.len() {
            return Err(buffer_too_small(end, out.len()));
        }
    ...
    let end = pos.checked_add(1).ok_or_else(|| overflow("pairs JSON closing brace"))?;
    if end > out.len() {
        return Err(buffer_too_small(end, out.len()));
    }
