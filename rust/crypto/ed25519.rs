...
const SPKI_HEADER: [u8; 12] = [0x30, 0x2a, 0x30, 0x05, 0x06, 0x03, 0x2b, 0x65, 0x70, 0x03, 0x21, 0x00];
...
pub fn raw_public_key(public_der: &[u8]) -> Option<&[u8]> {
    match public_der.len() {
        ...
        ED25519_SPKI_LEN if public_der.starts_with(&SPKI_HEADER) => Some(&public_der[SPKI_HEADER.len()..]),
        _ => None,
    }
}
...
pub fn generate_ed25519_keypair() -> Result<Ed25519Keypair> {
    let (private_key, public_key) = generate_keypair().ok_or_else(|| {
        Error::from_reason("ed25519 keypair generation failed (CSPRNG error)")
    })?;
    Ok(Ed25519Keypair {
