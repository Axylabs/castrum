// rust/ingress/api.rs — the `Ingress` napi class + `IngressInner` state.
//
// The napi boundary of the ingress pipeline. `IngressInner` holds the compiled
// per-instance configuration (options, CORS, rate limiter, schema, limits); the
// `Ingress` class exposes the packed and full-sync request entry points. The
// core 8-stage pipeline logic lives in `pipeline.rs` (`IngressInner`'s
// `handle_packed`); this module only marshals options into state and routes
// requests into it. Kept as a separate file so `ingress/mod.rs` stays a pure
// declaration hub.
...
use super::{IngressOptions, IngressSchema, Limits};
...
pub(crate) fn clamp_output_size(output_buffer_size: Option<u32>) -> usize {
    output_buffer_size
        .unwrap_or(262_144)
        .clamp(OUT_DATA_START as u32, MAX_OUTPUT_BUFFER_SIZE) as usize
}

/// Compiled per-instance ingress state. Constructed once by [`Ingress::new`]
/// from the JS-facing options; `handle_packed` (in `pipeline.rs`) runs the
/// 8-stage pipeline over this immutable configuration.
#[derive(Clone)]
...
impl Ingress {
    ...
    pub(crate) fn full_sync_into(
        ...
        let packed = crate::ingress::packed::build_packed_input_sync(
            method_kind as u8,
            url.as_bytes(),
            ip.as_bytes(),
            request_id.as_bytes(),
