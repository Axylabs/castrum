...
// instruction cache. They build responses via `HeaderFields` and never fail
// (writers are capacity-checked), so they return the written byte count
...
#[cold]
pub(crate) fn terminal_simple(out: &mut [u8], error_code: u8, status: u16) -> usize {
    HeaderFields::terminal(1, error_code, status, 0, RateInfo::default(), HV_JSON).write(out)
}

/// Write a preflight OK response (204).
#[cold]
pub(crate) fn terminal_preflight_ok(flags: u32, hv: u8, out: &mut [u8]) -> usize {
    HeaderFields::terminal(2, ERR_CODE_NONE, 204, flags, RateInfo::default(), hv).write(out)
}

/// Write a preflight forbidden response (403).
#[cold]
pub(crate) fn terminal_preflight_forbidden(flags: u32, hv: u8, out: &mut [u8]) -> usize {
    HeaderFields::terminal(
        2,
...
pub(crate) fn terminal_schema_validation(
    ...
    HeaderFields::terminal(1, ERR_CODE_SCHEMA_VALIDATION, 422, flags, rate, hv).write(out)
}
...
mod tests {
    ...
    fn terminal_simple_writes_error_layout() {
        ...
        let written = terminal_simple(&mut out, ERR_CODE_INVALID_JSON, 400);
    ...
    fn terminal_rate_limited_writes_429_and_rate_info() {
        ...
        let written = terminal_rate_limited(0, HV_JSON, rate, &mut out);
        assert_eq!(written, OUT_DATA_START);
        assert_eq!(out[OUT_VERDICT], 1);
    ...
    fn terminal_preflight_ok_is_204() {
        let mut out = vec![0u8; 128];
        let written = terminal_preflight_ok(0, HV_CORS_PREFLIGHT, &mut out);
        assert_eq!(written, OUT_DATA_START);
