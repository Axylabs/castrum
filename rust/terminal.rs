...
// Extracted from ingress.rs. These functions are #[cold] so they don't 
// pollute the hot-path instruction cache.
...
#[cold]
pub fn terminal_simple(out: &mut [u8], verdict: u8, error_code: u8, status: u16) -> Result<usize> {
...
#[cold]
pub fn terminal_preflight_ok(flags: u32, hv: u8, out: &mut [u8]) -> Result<usize> {
    Ok(write_output_header(
...
#[cold]
pub fn terminal_preflight_forbidden(flags: u32, hv: u8, out: &mut [u8]) -> Result<usize> {
    Ok(write_output_header(
