...
fn sign_cookie_batch_bytes(data: &[u8], secret: &[u8]) -> Result<Vec<u8>> {
    ...
    parse_batch_direct(
        &items,
        &mut out,
        |v| Ok(crate::crypto::cookie_sign::sign_cookie_bytes(v, &key)),
    );
    Ok(out)
}
...
#[napi] pub fn hex_encode_batch_packed(input: Uint8Array) -> Result<Buffer> { Ok(Buffer::from(hex_encode_batch_bytes(input.as_ref())?)) }
#[napi] pub fn hex_decode_batch_packed(input: Uint8Array) -> Result<Buffer> { Ok(Buffer::from(hex_decode_batch_bytes(input.as_ref())?)) }
#[napi]
pub fn base64_encode_batch_packed(input: Uint8Array, url_safe: Option<bool>, padding: Option<bool>) -> Result<Buffer> {
...
#[napi]
pub fn base64_decode_batch_packed(input: Uint8Array, url_safe: Option<bool>, padding: Option<bool>) -> Result<Buffer> {
    let us = url_safe.unwrap_or(false);
    let pad = padding.unwrap_or(true);
    let items = unpack(input.as_ref())?;
    let mut out = Vec::with_capacity(items.len() * 32);
...
mod tests {
    ...
    fn hex_encode_batch_matches_scalar() {
        ...
        assert_eq!(results[0], crate::crypto::base64::hex_encode_bytes(b"hi\0\xff").into_bytes());
