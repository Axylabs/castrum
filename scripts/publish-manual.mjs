...
function syncChangelog(version) {
  ...
  if (!text.includes(marker)) {
    console.warn(
      `publish-manual: WARNING: ${file} has no "${marker}" section — skipping changelog update.`,
    )
    return
  }
...
function ensureTagPushed() {
  ...
  if (branch !== 'HEAD') {
    try {
      ...
      console.warn(
        `publish-manual: WARNING: could not push branch '${branch}'. Push it later: git push origin ${branch}`,
      )
    }
  }
}
...
function main() {
  ...
  if (INCREMENT) {
    if (DRY_RUN) {
      log(
        `dry-run: would run \`bun pm version ${INCREMENT}\`, sync Cargo.toml/Cargo.lock/CHANGELOG.md,`,
      )
