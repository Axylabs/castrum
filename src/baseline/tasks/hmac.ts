...
export function nativeHmacSha256(key: Uint8Array, data: Uint8Array): Uint8Array {
  const hex = createHmac('sha256', Buffer.from(key)).update(Buffer.from(data)).digest('hex')
...
export function nativeHmacSha256Verify(
  ...
  const expected = createHmac('sha256', Buffer.from(key)).update(Buffer.from(data)).digest()
