...
export function nativeNegotiateEncoding(supported: string[], header: string): string | null {
  ...
  for (const sup of supported) {
    ...
    for (const p of prefs) {
      ...
      if (spec === 0) continue
      if (!matched || spec > matched.spec || (spec === matched.spec && p.order < matched.order)) {
