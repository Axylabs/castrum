...
export function runCorrectnessChecks(f: BenchFixtures): void {
  ...
  assertEqual(native.nativeJsonSum(f.jsonPayload), rust.jsonSumIds(f.jsonPayload), 'json sum')
  ...
  assertEqual(schemaValidator.validate(f.schemaDoc), true, 'schema: rust accepts valid doc')
  assertEqual(
    nativeJsonSchemaValidate(f.schemaBadDoc, f.jsonSchema),
    false,
    'schema: native rejects bad doc',
  )
  assertEqual(schemaValidator.validate(f.schemaBadDoc), false, 'schema: rust rejects bad doc')
  assertEqual(
  ...
  assertEqual(
    ...
    nativeIsNotModified(etagStr, f.httpDateSecs, null, decoder.decode(f.ifModifiedSinceHeader)),
  ...
  assertEqual(
    ...
    nativeNegotiateEncoding(['gzip', 'br', 'identity'], decoder.decode(f.acceptEncodingHeader)),
  ...
  assertEqual(
    rust.batch.verifyCookie([signedCookie, encoder.encode('bad.deadbeef')], f.cookieSecret)[0],
    1,
  ...
  assertEqual(
    rust.batch.csrfVerify([csrfTok, encoder.encode('bad.bad')], f.csrfSecret)[0],
    1,
    'csrf batch bitset',
  )
  assertEqual(
  ...
  assertEqual(native.nativeValidateEmail(f.emailOk), rust.validateEmail(f.emailOk), 'email valid')

  assertEqual(native.nativeValidateUuid(f.uuidOk), rust.validateUuid(f.uuidOk), 'uuid valid')

  assertEqual(native.nativeValidateIpv4(f.ipv4Ok), rust.validateIpv4(f.ipv4Ok), 'ipv4 valid')

  assertEqual(native.nativeValidateIpv6(f.ipv6Ok), rust.validateIpv6(f.ipv6Ok), 'ipv6 valid')

  assertEqual(native.nativeCrc32(f.crcInput), rust.crc32(f.crcInput), 'crc32')

  assertEqual(native.nativeFnv1a64(f.crcInput), rust.fnv1a64(f.crcInput), 'fnv1a64')

  assertEqual(native.nativeJsonValid(f.jsonPayload), rust.jsonValid(f.jsonPayload), 'json valid')
  ...
  const nativeToken = native.nativeJwtSign(f.jwtClaims, f.jwtSecret, 3600, f.jwtNowSeconds)
  assertEqual(
  ...
  tamperedToken[tamperedToken.length - 1] = (tamperedToken[tamperedToken.length - 1] ?? 0) ^ 0x01
  assertEqual(
  ...
  const nativeParts = native.nativeMultipartParse(f.multipartBody, f.multipartBoundary)
  ...
  assertEqual(rustParts[1]?.filename, nativeParts[1]?.filename, 'multipart filename')
  assertDeepEqual(rustParts[1]?.data, nativeParts[1]?.data, 'multipart file data')
  ...
  assertDeepEqual(rust.wsFrameDecode(rustWs)?.payload, f.wsPayload, 'ws frame decode roundtrip')
...
export function runComplexCorrectnessChecks(_f: BenchFixtures, c: ComplexFixtures): void {
  // Batch JSON validation
  const nativeBatch = c.batchJsonDocs.reduce(
    (acc, doc) => acc + (native.nativeJsonValid(doc) ? 1 : 0),
    0,
  )
  const rustBatchTest = c.batchJsonDocs.reduce((acc, doc) => acc + (rust.jsonValid(doc) ? 1 : 0), 0)
  ...
  const rustEmails = c.batchEmails.reduce((acc, e) => acc + (rust.validateEmail(e) ? 1 : 0), 0)
  ...
  const rustUuids = c.batchUuids.reduce((acc, u) => acc + (rust.validateUuid(u) ? 1 : 0), 0)
  ...
  const rustIps = c.batchIpv4s.reduce((acc, ip) => acc + (rust.validateIpv4(ip) ? 1 : 0), 0)
  assertEqual(nativeIps, rustIps, 'batch ipv4 valid')

  // Large JSON validation
  assertEqual(native.nativeJsonValid(c.jsonLarge), rust.jsonValid(c.jsonLarge), 'large json valid')

  // Huge JSON validation
  assertEqual(native.nativeJsonValid(c.jsonHuge), rust.jsonValid(c.jsonHuge), 'huge json valid')
  ...
  const rustBatchJson = rustBatch.jsonValid(c.batchJsonDocs).reduce((acc, b) => acc + b, 0)
  assertEqual(nativeBatch, rustBatchJson, 'batch json valid packed')

  const rustBatchEmails = rustBatch.validateEmail(c.batchEmails).reduce((acc, b) => acc + b, 0)
  assertEqual(nativeEmails, rustBatchEmails, 'batch email valid packed')

  const rustBatchUuids = rustBatch.validateUuid(c.batchUuids).reduce((acc, b) => acc + b, 0)
  assertEqual(nativeUuids, rustBatchUuids, 'batch uuid valid packed')

  const rustBatchIpv4s = rustBatch.validateIpv4(c.batchIpv4s).reduce((acc, b) => acc + b, 0)
  ...
  const loaderEmailSum = loader('validateEmail')(c.batchEmails).reduce((acc, b) => acc + b, 0)
  const rustEmailSum = rust.batch.validateEmail(c.batchEmails).reduce((acc, b) => acc + b, 0)
