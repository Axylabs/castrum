...
export function nativeSignCookie(value: Uint8Array, secret: Uint8Array): Uint8Array {
  const sig = createHmac('sha256', Buffer.from(secret)).update(Buffer.from(value)).digest('hex')
...
export function nativeVerifyCookie(signed: Uint8Array, secret: Uint8Array): Uint8Array | null {
  let dot = -1
  ...
  const expected = createHmac('sha256', Buffer.from(secret)).update(Buffer.from(value)).digest()
