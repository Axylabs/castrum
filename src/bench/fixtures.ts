...
export function createFixtures(): BenchFixtures {
  ...
  const jsonPatch = encoder.encode(JSON.stringify([{ op: 'replace', path: '/a', value: 42 }]))
  ...
  const aeadPlaintext = encoder.encode('sensitive session payload for the encryption benchmark')
  ...
  const templateSource = '{% for u in users %}<li>{{ u.name }} ({{ u.id }})</li>\n{% endfor %}'
  ...
  const schemaBadDoc = encoder.encode(JSON.stringify({ id: 'nope', name: 42, extra: true }))
  ...
  // ── ETag / HTTP-date / conditional (M3) ──
  const etagData = encoder.encode('the quick brown fox jumps over the lazy dog 1234567890')
  const httpDateSecs = 784111777 // Sun, 06 Nov 1994 08:49:37 GMT
  const ifNoneMatchHeader = encoder.encode('"abc123", W/"def456"')
  const ifModifiedSinceHeader = encoder.encode('Sun, 06 Nov 1994 08:49:37 GMT')

  // ── Accept-Encoding negotiation (M4) ──
  const acceptEncodingHeader = encoder.encode('gzip;q=0.8, deflate, br;q=1.0, *;q=0.1')
...
export function createComplexFixtures(): ComplexFixtures {
  ...
  const httpHuge = encoder.encode(
    'GET /api/search?q=' +
      ...
      ' HTTP/1.1\r\n' +
      'Host: example.com\r\n' +
      Array.from({ length: 50 }, (_, i) => `X-Custom-Header-${i}: value-${i}\r\n`).join('') +
      '\r\n',
  )

  const cookieLarge = encoder.encode(
    Array.from({ length: 50 }, (_, i) => `cookie_${i}=value_${i}_with_some_extra_data_${i}`).join(
      '; ',
    ),
  )

  const queryComplex = encoder.encode(
    Array.from({ length: 100 }, (_, i) => `param${i}=value${i}&arr[]=${i}a&arr[]=${i}b`).join('&'),
  )
  ...
  const batchUuids = Array.from({ length: 100 }, (_, i) =>
    encoder.encode(`550e8400-e29b-41d4-a716-${String(i).padStart(12, '0')}`),
  )
  ...
  const batchQueries = Array.from({ length: 100 }, (_, i) =>
    encoder.encode(`id=${i}&name=user${i}&active=${i % 2 === 0}&tags[]=${i}a&tags[]=${i}b`),
  )
  ...
  const batchTokens = Array.from({ length: 100 }, (_, i) =>
    nativeJwtSign({ sub: String(i), name: `user${i}`, role: 'admin' }, batchSecret, 3600, batchNow),
  )
