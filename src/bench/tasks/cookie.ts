...
export function cookieTasks(f: BenchFixtures): BenchTask[] {
  return [
    ...
    },
    {
      ...
      run: () => {
        const obj = pairsToObject(readPairsPacked(native.nativeCookieParsePacked(f.cookieStr)))
    ...
    },
    {
      ...
      run: () => {
        const obj = pairsToObject(readPairsPacked(rust.cookieParsePacked(f.cookieStr)))
        return Object.keys(obj).length
      },
      iterations: 300,
      warmup: 30,
