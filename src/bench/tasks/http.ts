...
export function httpTasks(f: BenchFixtures): BenchTask[] {
  return [
    ...
    },
    {
      ...
      run: () => {
        const parsed = readHttpPacked(native.nativeHttpParseRequestPacked(f.httpRaw))
        return parsed.method.length + parsed.path.length + Object.keys(parsed.headers).length
      },
      iterations: 300,
      warmup: 30,
    },
