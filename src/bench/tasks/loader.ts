...
export function loaderTasks(c: ComplexFixtures): BenchTask[] {
  ...
  return [
    ...
    },
    {
      ...
      run: () =>
        hmac(c.batchTokens, c.batchSecret).reduce(
    ...
    },
    {
      ...
      run: () =>
        c.batchTokens.reduce(
          (acc, t) => acc + rust.hmacSha256(c.batchSecret, t).byteLength,
          0,
        ),
      iterations: 200,
      warmup: 20,
    ...
    },
    {
      ...
      run: () =>
        jsonValid(c.batchJsonDocs).reduce((acc, b) => acc + b, 0),
      iterations: 200,
      warmup: 20,
    },
    {
    ...
    // ── Adaptive (learning on): dispatch overhead vs the pinned loader ──
    {
      ...
      run: () =>
        adaptiveEmail(c.batchEmails).reduce((acc, b) => acc + b, 0),
      iterations: 1000,
      warmup: 100,
    },
    ...
    // ── Expanded-op bulk coverage: one packed call vs per-item scalar loop ──
    {
      ...
      run: () =>
        l("fnv1a64")(c.batchTokens).reduce((acc, v) => acc + v, 0n),
      iterations: 1000,
      warmup: 100,
    },
    {
    ...
    },
    {
      ...
      run: () =>
        l("etag")(c.batchTokens).reduce((acc, b) => acc + b.byteLength, 0),
      iterations: 1000,
      warmup: 100,
    },
    {
      ...
      run: () =>
        c.batchTokens.reduce((acc, t) => acc + rust.etag(t).byteLength, 0),
      iterations: 1000,
      warmup: 100,
    },
    {
      ...
      run: () =>
        l("base64UrlEncode")(c.batchTokens).reduce(
          (acc, b) => acc + b.byteLength,
          0,
        ),
      iterations: 1000,
    ...
    },
    {
      ...
      run: () =>
        c.batchTokens.reduce(
          (acc, t) => acc + rust.base64UrlEncode(t).byteLength,
          0,
        ),
      iterations: 1000,
      warmup: 100,
...
export async function runLoaderAsyncBenchmarks(
  ...
  for (let r = 0; r < rounds; r++) {
    const results = await Promise.all(
      compressItems.map((item) => gzip.load(item)),
    );
