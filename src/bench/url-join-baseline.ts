...
export function nativeUrlResolve(base: Uint8Array, reference: Uint8Array): Uint8Array {
...
export function nativeUrlEncodeQuery(params: Record<string, string>): Uint8Array {
  const keys = Object.keys(params).sort()
  const parts: string[] = []
  for (const key of keys) {
    parts.push(`${encodeURIComponent(key)}=${encodeURIComponent(params[key] as string)}`)
  }
