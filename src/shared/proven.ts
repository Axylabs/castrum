...
export const PROVEN_SURFACE = [
  ...
  { name: "jsonPatch", label: "JSON Patch", nativeTask: "native:json_patch", rustTask: "rust:json_patch", status: "proven", typicalRatio: 1.2, note: "Modest but real win; swings ~0.9-1.5x run-to-run (DOM marshal dominated)." },
  ...
  { name: "urlEncode", label: "URL encode", nativeTask: "native:url_encode", rustTask: "rust:url_encode", status: "not-competitive", note: "Delegates to encodeURIComponent under Bun (BUN_WINS — ~3-4x faster than the FFI crossing; skips the C-ABI call). Still ~1.5-1.8x behind the native string baseline because the byte API pays a bytes→string→bytes round-trip; the rust addon remains the Node path. Byte parity pinned by test/unit/features/url.test.ts." },
  { name: "urlDecode", label: "URL decode", nativeTask: "native:url_decode", rustTask: "rust:url_decode", status: "not-competitive", note: "Delegates to decodeURIComponent under Bun (BUN_WINS — ~4-8x faster than the FFI crossing; strict UTF-8 semantics match: both throw on malformed/invalid). Rust addon remains the Node path and the raw-bytes urlDecodeBytes stays native. Parity pinned by test/unit/features/url.test.ts." },
  ...
  { name: "gzipDecompress", label: "Gzip decompress", nativeTask: "native:gzip_decompress", rustTask: "rust:gzip_decompress", status: "proven", typicalRatio: 1.4, note: "zlib-rs vs node zlib; swings 1.0-1.4x run-to-run." },
  ...
  { name: "createSchemaValidator", label: "JSON schema validate", nativeTask: "native:json_schema_validate", rustTask: "rust:json_schema_validate", status: "proven", note: "Zero-DOM fast path validates raw bytes for the common keyword subset (scalar ~1.2-2.4x, batch ~1.0-1.56x vs ajv); unsupported keywords fall back to the jsonschema crate." },

  // ── Parity (no clear win) ──
  { name: "crc32", label: "CRC32", nativeTask: "native:crc32", rustTask: "rust:crc32", status: "not-competitive", note: "Sub-µs FFI-crossing bound vs the same-engine JS crc-32 loop (~0.23-0.44x across runs); use the packed crc32BatchPacked path which wins on larger inputs." },
  { name: "xxh3", label: "XXH3-64", nativeTask: "diag:bun_hash_xxh3", rustTask: "diag:xxh3", status: "not-competitive", note: "Bun.hash.xxHash3 wins ~4x (in-process C++ beats the FFI crossing). Keep the export as the non-Bun/Node fast path; @flux/native should prefer Bun.hash.xxHash3 under Bun. See docs/bun-builtins-decision-matrix.md." },
  ...
  { name: "passwordHashBcrypt", label: "Password hash (bcrypt)", nativeTask: "diag:bun_password_bcrypt_hash", rustTask: "diag:bcrypt_hash", status: "parity", note: "Bun.password.bcrypt ~1.24x faster at cost 10 (noisy KDF); rust is self-contained and non-Bun. See docs/bun-builtins-decision-matrix.md." },
  { name: "passwordVerifyBcrypt", label: "Password verify (bcrypt)", nativeTask: "diag:bun_password_bcrypt_verify", rustTask: "diag:bcrypt_verify", status: "parity", note: "Rust ~1.49x vs Bun.password.verify at cost 10; KDF timings noisy — parity for gate stability." },
  { name: "pbkdf2Sha256", label: "PBKDF2-HMAC-SHA256", nativeTask: "diag:pbkdf2_sha256", rustTask: "diag:pbkdf2_sha256_rust", status: "parity", note: "~1.08x vs node:crypto pbkdf2Sync at 100k rounds (parity); Bun has no synchronous PBKDF2." },
  ...
  { name: "parseMediaType", label: "Media type parse", nativeTask: "native:media_type_parse", rustTask: "rust:media_type_parse", status: "not-competitive", note: "Consistent ~0.71-0.81x (sub-µs FFI crossing vs the JS baseline); use MediaTypeMatcher (precompiled expected) for the zero-alloc match path." },
  { name: "etag", label: "ETag", nativeTask: "native:etag", rustTask: "rust:etag", status: "parity", typicalRatio: 1.06 },
  { name: "httpDate", label: "HTTP date", nativeTask: "native:http_date", rustTask: "rust:http_date", status: "not-competitive", note: "Delegates to Date.toUTCString() under Bun (BUN_WINS — ~3.7x faster than the FFI crossing; byte-identical RFC 1123 for HTTP timestamps, incl. leap years/epoch). Rust addon remains the Node path; pooled httpDateInto stays native." },
  ...
  { name: "base64Encode", label: "Base64 encode", nativeTask: "native:base64_encode", rustTask: "rust:base64_encode", status: "not-competitive", note: "Delegates to Buffer base64 under Bun for the standard padded, non-url-safe case (BUN_WINS — ~2x faster than the FFI crossing); url-safe/unpadded falls through to native. Rust addon remains the Node path. Parity pinned by test/unit/features/encoding.test.ts." },
  ...
  { name: "hexEncode", label: "Hex encode", nativeTask: "native:hex_encode", rustTask: "rust:hex_encode", status: "not-competitive", note: "Buffer.toString('hex') is native SIMD; pooled hexEncodeInto cuts the loss from ~2.2x to ~1.35x." },
  ...
  { name: "urlResolve", label: "URL resolve", nativeTask: "native:url_resolve", rustTask: "rust:url_resolve", status: "parity", typicalRatio: 1.11, note: "Noisy ~1.5µs op; swings 0.74-1.03x run-to-run. UrlBuilder reuses the parsed base." },
