...
// Boot randomness: one webcrypto draw, falling back to Math.random for hosts
// (older Node) without a `crypto` global. `[0]` is `number | undefined` under
// noUncheckedIndexedAccess — the `??` supplies the fallback for that too.
const BOOT_RANDOM =
  typeof crypto !== 'undefined'
    ? (crypto.getRandomValues(new Uint32Array(1))[0] ?? (Math.random() * 0xffffffff) >>> 0)
