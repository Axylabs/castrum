...
import {
  nativeSignCookie,
  nativeVerifyCookie,
} from "../../../src/bench/cookie-sign-baseline";
...
describe("CookieSigner (higher-order instance)", () => {
  ...
  test("sign matches the node:crypto baseline byte-for-byte", () => {
    expect(Array.from(signer.sign(VALUE))).toEqual(
      Array.from(nativeSignCookie(VALUE, SECRET)),
    );
  });

  test("verify roundtrips the value", () => {
    const signed = signer.sign(VALUE);
    expect(Array.from(signer.verify(signed) ?? new Uint8Array(0))).toEqual(
      Array.from(VALUE),
    );
  });
...
describe("rust.signCookie / verifyCookie (scalar)", () => {
  test("matches baseline", () => {
    ...
    expect(
      Array.from(
        rust.verifyCookie(rust.signCookie(VALUE, SECRET), SECRET) ??
          new Uint8Array(0),
      ),
