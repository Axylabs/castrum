...
const BODY = encoder.encode(
  "name=John+Doe&email=john%40example.com&age=30&tags=a&tags=b&empty=",
);
...
describe("rust.formParsePacked", () => {
  test("parses to pairs matching URLSearchParams", () => {
    ...
    const nativeObj = pairsToObject(
      readPairsPacked(nativeFormParsePacked(BODY)),
    );
  ...
  test("handles empty body", () => {
    expect(
      pairsToObject(readPairsPacked(rust.formParsePacked(new Uint8Array(0)))),
    ).toEqual({});
  });
});
