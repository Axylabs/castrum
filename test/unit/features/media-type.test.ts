...
describe("rust.parseMediaType", () => {
  ...
  test("lowercases type/subtype and param names", () => {
    const m = rust.parseMediaType(
      encoder.encode("Application/JSON; Charset=UTF-8"),
    );
...
describe("MediaTypeParser (higher-order instance)", () => {
  test("matches wildcards", () => {
    ...
    expect(parser.matches(JSON_CT, encoder.encode("application/xml"))).toBe(
      false,
    );
  });
  ...
  test("matches parity with the JS baseline", () => {
    ...
    for (const expected of [
      ...
      expect(
        parser.matches(encoder.encode(actual), encoder.encode(expected)),
      ).toBe(nativeMediaTypeMatches(actual, expected));
    }
  });
});
