...
describe('createWebSocketUpgrade', () => {
  test('returns a 101 with the RFC 6455 accept key', () => {
    ...
    expect(up?.response.headers.get('sec-websocket-accept')).toBe('s3pPLMBiTxaQ9kYGzzhZRbK+xOo=')
    expect(up?.key).toBe('dGhlIHNhbXBsZSBub25jZQ==')
    expect(up?.protocol).toBeNull()
  })

  test('returns null when the key is missing', () => {
    const up = createWebSocketUpgrade(new Request('http://localhost:9999/ws', { headers: {} }))
    expect(up).toBeNull()
  })

  test('returns null when the key is empty', () => {
    const up = createWebSocketUpgrade(wsReq('/ws', { 'sec-websocket-key': '' }))
    expect(up).toBeNull()
  })
  ...
  test('protocol negotiation is case-insensitive', () => {
    const up = createWebSocketUpgrade(wsReq('/ws', { 'sec-websocket-protocol': 'CHAT' }), {
      protocols: ['chat'],
    })
    expect(up?.protocol).toBe('chat')
  })
  ...
  test('does not echo an unsupported subprotocol', () => {
    const up = createWebSocketUpgrade(wsReq('/ws', { 'sec-websocket-protocol': 'bogus' }), {
      protocols: ['chat'],
    })
    expect(up?.protocol).toBeNull()
