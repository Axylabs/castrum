...
describe('rust.multipartParse', () => {
  test('parses parts with names, filename, content-type', () => {
    ...
    expect(toBytes(parts[1]?.data)).toEqual([...encoder.encode('file contents here')])
  })
...
describe('rust.multipartParsePacked (zero-copy scalar)', () => {
  test('packed output matches the object path', () => {
    ...
    for (const p of parts) {
      ...
      pos += 4
      expect(hasFile ? decoder.decode(packed.subarray(pos, pos + fileLen)) : '').toBe(
        p.filename ?? '',
      )
      pos += fileLen
      ...
      pos += 4
      expect(hasCt ? decoder.decode(packed.subarray(pos, pos + ctLen)) : '').toBe(
        p.contentType ?? '',
      )
      pos += ctLen
