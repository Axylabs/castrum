...
describe("task aggregation", () => {
  test("groups the canonical comparison with `_`-suffixed variants", () => {
    ...
    expect(classified.tasks.some((t) => t.isVariant && t.rustName === "rust:json_valid_batch")).toBe(true);
  });
  ...
  test("excludes variants claimed by another registry entry (jwt_sign vs jwt_sign_bytes)", () => {
    ...
    const forJwtSign = classifyEntry(jwtSign, [cmp("rust:jwt_sign", 0.5), cmp("rust:jwt_sign_bytes", 1.07)], claimed);
    const forBytes = classifyEntry(jwtSignBytes, [cmp("rust:jwt_sign", 0.5), cmp("rust:jwt_sign_bytes", 1.07)], claimed);
...
describe("unmeasured / missing tasks", () => {
  test("entry without a rustTask is unmeasured", () => {
    const classified = classifyEntry(
      provenEntry({ rustTask: "", status: "unmeasured" }),
      [cmp("rust:fn", 3.0)],
    );
...
describe("tolerances and formatting", () => {
  test("task bands: won = ratio>=1, failed = clear loss, middle = tie", () => {
    ...
    const clearLoss = classifyEntry(provenEntry(), [
      cmp("rust:fn", 1 / LOSS_TOLERANCE - 0.01),
    ]);
